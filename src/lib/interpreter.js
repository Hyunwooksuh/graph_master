...
Interpreter.prototype.parse_ = function (code, sourceFile) {
  ...
  const options = {};
  for (const name in Interpreter.PARSE_OPTIONS) {
...
Interpreter.prototype.initGlobal = function (globalObject) {
  ...
  const func = this.createNativeFunction((_x) => {
...
Interpreter.prototype.initFunction = function (globalObject) {
  ...
  wrapper = function apply(thisArg, args) {
    const state = thisInterpreter.stateStack[thisInterpreter.stateStack.length - 1];
...
Interpreter.prototype.initObject = function (globalObject) {
  ...
  wrapper = function getOwnPropertyNames(obj) {
    ...
    const props = obj instanceof Interpreter.Object ? obj.properties : obj;
...
Interpreter.prototype.initRegExp = function (globalObject) {
  ...
  wrapper = function RegExp(pattern, flags) {
    ...
    thisInterpreter.populateRegExp(rgx, new Interpreter.nativeGlobal.RegExp(pattern, flags));
    return rgx;
  };
...
Interpreter.prototype.initJSON = function (globalObject) {
  ...
  let wrapper = function parse(text) {
    try {
...
Interpreter.prototype.populateError = function (pseudoError, opt_message) {
  ...
  const message = String(this.getProperty(pseudoError, "message"));
  let stackString = `${name}: ${message}\n`;
  for (var i = 0; i < tracebackData.length; i++) {
    const { loc } = tracebackData[i];
    var { name } = tracebackData[i];
...
Interpreter.prototype.nativeToPseudo = function (nativeObj) {
  ...
  if (typeof nativeObj === "function") {
    ...
    const wrapper = function () {
      const args = Array.prototype.slice.call(arguments).map((i) => {
...
Interpreter.prototype.getScope = function () {
  const { scope } = this.stateStack[this.stateStack.length - 1];
  if (!scope) {
...
Interpreter.prototype.createGetter_ = function (func, left) {
  ...
  const state = new Interpreter.State(node, this.stateStack[this.stateStack.length - 1].scope);
...
Interpreter.prototype["stepAssignmentExpression"] = function (stack, state, node) {
  if (!state.doneLeft_) {
    ...
    const nextState = new Interpreter.State(node["left"], state.scope);
...
Interpreter.prototype["stepCallExpression"] = function (stack, state, node) {
  ...
  if (!state.doneExec_) {
    ...
    if (func.eval) {
      ...
      const argsWithCallback = state.arguments_.concat(new Array(argLength)).slice(0, argLength);
...
Interpreter.prototype["stepForInStatement"] = function (stack, state, node) {
  ...
  if (!state.doneVariable_) {
    ...
    if (left["type"] === "VariableDeclaration") {
      ...
      const nextState = new Interpreter.State(left, state.scope);
      nextState.components = true;
      return nextState;
    }
  }
...
Interpreter.prototype["stepLabeledStatement"] = function (stack, state, node) {
  ...
  const nextState = new Interpreter.State(node["body"], state.scope);
...
Interpreter.prototype["stepTryStatement"] = function (stack, state, node) {
  ...
  if (
    ...
    const nextState = new Interpreter.State(node["handler"], state.scope);
...
Interpreter.prototype["stepUnaryExpression"] = function (stack, state, node) {
  if (!state.done_) {
    state.done_ = true;
    const nextState = new Interpreter.State(node["argument"], state.scope);
